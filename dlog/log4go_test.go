...
var formatTests = []struct {
...
}{
	{
		...
		Formats: map[string]string{
			...
			FORMAT_DEFAULT: "[20090213 23:31:30] [ERROR] (source) message\n",
...
func TestFormatLogRecord(t *testing.T) {
	...
	for _, test := range formatTests {
		...
		for fmt, want := range test.Formats {
			if got := FormatLogRecord(&cache, fmt, test.Record); got != want {
...
var logRecordWriteTests = []struct {
...
}{
	{
		...
		Console: "[23:31:30 20090213] [FATAL] (source) message\n",
	},
}

func _TestConsoleLogWriter(t *testing.T) {
...
func TestFileLogWriter(t *testing.T) {
	...
	w := NewFileLogWriter(testLogFile, false, false, true)
	if w == nil {
...
func TestLogOutput(t *testing.T) {
	const (
		expected = "7ebaf27d579c5577f41b657772346d9f"
	)
	...
	l.AddFilter("file", FINEST, NewFileLogWriter(testLogFile, false, false, false).SetFormat("[%L] %M"))
	defer os.Remove(testLogFile)
...
func BenchmarkFormatLogRecord(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		if i%2 == 0 {
			FormatLogRecord(&cache, FORMAT_DEFAULT, rec)
		} else {
			FormatLogRecord(&cache, FORMAT_SHORT, rec)
		}
	}
}
...
func BenchmarkConsoleLog(b *testing.B) {
	...
	sink, err := runtime.Open(runtime.DevNull)
	if err != nil {
		panic(err)
	}
...
func BenchmarkFileLog(b *testing.B) {
	sl := make(Logger)
	b.StopTimer()
	sl.AddFilter("file", INFO, NewFileLogWriter("benchlog.log", false, false, false))
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		sl.Log(WARNING, "here", "This is a log message")
	}
